...
class Form {
	constructor(server) {
		...
		if (cfg.status !== "on") this.maxBytes = 0;

		this.uploaded = false;

		this.headers(() => server.req.on("data", data => this.write(data)).on("end", () => this.end()));
	}
	...
	headers(cb) {
		...
		if (header && header.match(/urlencoded/i)) this.type = "urlencoded";
	...
	write(data) {
		...
		if (this.server.res.finished) return;

		if (this.type === "urlencoded") this.urlencoded();

		if (this.type === "multipart") this.multipart();
	}
	...
	urlencoded() {
		...
		for (i of buffer) {
			...
			if (array) {
				if (tmp[i[0]]) {
					if (typeof tmp[i[0]] === "string") tmp[i[0]] = [];
	...
	multipart(iterator = 0, state = 0) {
		...
		for (i = iterator; i < buffer.length; ++i) {
			...
			case states.BOUNDARY:
				if (bIdx && chr !== boundary[bIdx]) bIdx = 0;
			...
			case states.HEADER_START:
				if ((chr === 13 && buffer[i + 1] === 10) || (chr === 45 && buffer[i + 1] === 45)) {
					state = states.FIELD;
