...
module.exports = (root, data, headers) => {
	...
	sandbox.fileRename = (oldPath, newPath) => {
		if (!path.isAbsolute(oldPath)) {
			oldPath = path.join(includePath, oldPath);
		}

		if (!path.isAbsolute(newPath)) {
			newPath = path.join(includePath, newPath);
		}
	...
	sandbox.fileCopy = (src, target) => {
		...
		if (!path.isAbsolute(target)) {
			target = path.join(includePath, target);
		}
	...
	sandbox.fileExists = file => {
		if (!path.isAbsolute(file)) {
			file = path.join(includePath, file);
		}
