...
function strPad(str, len = 3, pad = "0", type = "left") {
	let tmp = ["", "", (str + "").length];

	while (tmp[2] < len) {
		if (type === "both") {
			if (tmp[2] + pad.length < len) {
				tmp[1] += pad;
				tmp[2] += pad.length;
			} else {
				tmp[1] += pad.substr(0, len - tmp[2]);
				tmp[2] += len - tmp[2];
			}
		}

		if (tmp[2] + pad.length < len) {
			tmp[type === "right" ? 1 : 0] += pad;
			tmp[2] += pad.length;
		} else {
			tmp[type === "right" ? 1 : 0] += pad.substr(0, len - tmp[2]);
			tmp[2] += len - tmp[2];
		}
	}
...
module.exports = (root, data, headers) => {
	...
	sandbox.fileRename = (oldPath, newPath) => {
		if (!path.isAbsolute(oldPath)) {
			oldPath = path.join(path.dirname(root.filename), oldPath);
		}

		if (!path.isAbsolute(newPath)) {
			newPath = path.join(path.dirname(root.filename), newPath);
		}

		fs.renameSync(oldPath, newPath);
	};

	sandbox.fileCopy = (src, target) => {
		if (!path.isAbsolute(src)) {
			src = path.join(path.dirname(root.filename), src);
		}

		if (!path.isAbsolute(target)) {
			target = path.join(path.dirname(root.filename), target);
		}
