...
class Viridine {
	...
	writeAccess(conn) {
		...
		if (this.terminal) {
			console.log("[access] Took", Date.now() - conn.SERVER.requestTime,
				"ms to send", conn.filename);
		}

		if (cfg.access.status === "on" &&
				!conn.findDenied(cfg.access.exclude)) {
			date = new Date(conn.SERVER.requestTime);
			date = date.toLocaleString("en-us");

			format = cfg.access.format;
			format = format.replace("$ipv4", conn.SERVER.remoteIpv4);
			format = format.replace("$ipv6", conn.SERVER.remoteIpv6);
			format = format.replace("$time", date);
			format = format.replace("$request", conn.req.url);
			format = format.replace("$status", conn.status);
			format = format.replace("$user_agent", conn.SERVER.httpUserAgent);
