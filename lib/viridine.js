...
class Viridine {
	...
	writeAccess(conn) {
		...
		if (this.terminal) {
			console.log("[access] Took",
				Date.now() - globalServer.requestTime,
		...
		if (cfg.access.status === "on" &&
			...
			format = format.replace("$ipv4", globalServer.remoteIpv4);
			format = format.replace("$ipv6", globalServer.remoteIpv6);
			format = format.replace("$time", date);
			format = format.replace("$request", conn.req.url);
			format = format.replace("$status", conn.status);
			format = format.replace("$user_agent", globalServer.httpUserAgent);
