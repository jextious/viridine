...
class Connection {
	...
	findDenied(cfg) {
		let denied = [
				"all",
				this.SERVER.remoteIpv4,
				this.SERVER.remoteIpv6,
				this.SERVER.httpUserAgent
			],
			deny = !cfg,
			i,
			j;
	...
	findHost(host) {
		...
		this.hostData = this.server.hostData[host];
	...
	fillServerGlobal() {
		...
		this.SERVER.documentRoot = path.join(process.cwd(), this.base);

		this.SERVER.httpHost = this.host;

		this.SERVER.httpReferer = this.req.headers.referer;

		this.SERVER.httpUserAgent = this.req.headers["user-agent"];

		this.SERVER.remoteIpv4 = ip.v4(this.req.connection.remoteAddress);

		this.SERVER.remoteIpv6 = this.req.connection.remoteAddress;

		this.SERVER.requestTime = Date.now();

		this.SERVER.requestUri = this.req.url;

		this.SERVER.scriptFilename = path.join(process.cwd(), this.filename);

		this.SERVER.serverPort = this.server.port;
		...
		if (this.form && this.form.uploaded) {
			for (i in this.FILES) {
				if (this.FILES.hasOwnProperty(i)) {
					if (this.FILES[i].length) {
						for (j of this.FILES[i]) {
							fs.unlink(j.path, cb);
						}
					} else {
						fs.unlink(this.FILES[i].path, cb);
					}
				}
			}
		}
